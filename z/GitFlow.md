...
tags:
  ...
  - type/concept
  - status/evergreen
description: Git Flow 工作流。2010 年，Vincent Driessen在博客文章《A successful Git branching model》中正式提出。这是最早被广泛采用的标准化 Git 工作流。
created: 2025-10-25T21:16:13
...
- **提出者：** **Vincent Driessen**
- **起源：** 2010 年，他在博客文章《A successful Git branching model》中正式提出。这是最早被广泛采用的标准化 Git 工作流。
- **核心特点：** 结构非常严谨且复杂。
    - **双主分支：** `master` (生产环境) 和 `develop` (开发环境)。
    - **辅助分支：** `feature` (功能)、`release` (预发布)、`hotfix` (热修复)。
- **适用场景：** 有严格“版本发布周期”的传统软件（如手机 App、桌面软件），因为它们需要很长的测试和稳定期。
- **缺点：** 分支太繁琐，合并容易冲突，不适合需要每天多次发布的互联网产品。
