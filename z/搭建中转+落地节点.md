...
tags:
  ...
  - type/howto
  - status/growing
description: 链式代理搭建-中转节点+落地节点配置
...
利用机场的专线 + 自己搭建的节点（使用ss协议即可） = 低延迟+干净（ip） 的网络环境
...
proxy-providers:
  Airport1:
    url: "http://192.168.10.42:3001/CKg2abstVnOeRpm1aB4G/download/DOG?target=ClashMeta"
    type: http
    interval: 86400
    health-check:
      enable: true
...
难点：
当你的落地节点为策略组时，落地节点的信息要把 自己按节点 筛选出来；**中转节点不能包含落地节点**（形成回环）



## 方法三（更灵活）

使用 sub-store 的组合订阅，将 落地节点 作为一个节点,
```yaml
# 节点信息
proxies:
  - name: "香港落地"
    ...
    udp: true
  - name: "xxx"
    dialer-proxy: hk 香港节点
```

dialer-proxy 的中转节点在 机场 配置文件里 一定要有
## 实战经验
...
https://sub.ba.cln.com/cD7wiTGit2/download/collection/%E4%B8%93%E7%BA%BF%2B%E8%90%BD%E5%9C%B0?target=ClshMeta
点击组合节点中间 选择 mihomo 的订阅连接（如上），结果导致生成的 land 组只有一个 Compatible 节点，而非两个 land 节点。问ai 说这绝对不是正确节点，后面试了右侧出现的复制订阅连接的按钮（如下），还真成功了
https://sub.ba.cln.com/cD7wiTGit2/download/collection/%E4%B8%93%E7%BA%BF%2B%E8%90%BD%E5%9C%B0

按道理 clash-party 是 mihomo 类型的吧
## 经验总结

### 测试

看 clash 的节点速率测试，结果好像经常直连比 中转落地 低，是 DO 直连太吊了吗？
打算用更全面的测试

中转后

![[Pasted image 20251126104608.png]]

直连
![[Pasted image 20251126110947.png]]

## 信息参考

https://www.youtube.com/watch?v=3oiA2Ym1Wu4
